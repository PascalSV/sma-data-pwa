...
app.use('*', async (c, next) => {
    ...
    const secret = c.env.SMA_DATA_SERVER_ACCESS;
...
app.get('/current-and-max', async (c) => {
    try {
        ...
        const latestValueSql = `
            SELECT strftime('%Y-%m-%d %H:%M:%S', a.TimeStamp, 'unixepoch') AS nice_timestamp, a.Serial, a.Power, a.TotalYield, a.LastChangedAt 
            FROM PascalsDayData a 
            INNER JOIN (
